...
pub const VALID_TEMPLATES: Vec<&str> = vec![
    "Basic", "Book", "Math", "Theatre", "Code", "Novel", "Beamer", "Lachaise",
];

pub const VALID_CLASSES: Vec<&str> = vec![
...
pub enum TexCreateError {
    ...
    #[fail(display = "The {} field has an empty value!")]
    EmptyFields(String),
    #[fail(display = "{}", _0)]
    IOError(#[cause] Error),
}

pub type Result<T> = std::result::Result<T, TexCreateError>;

pub fn check_beamer_error(config: &Config) -> Result<()> {
    /*
...
pub fn check_invalid_template(config: &Config) -> Result<()> {
    /*
...
pub fn check_invalid_class(config: &Config) -> Result<()> {
    /*
...
pub fn check_empty_field(config: &Config) -> Result<()> {
    /*
...
pub fn check_errors(config: &Config) -> Result<()> {
    /*
